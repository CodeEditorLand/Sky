...
const isSaveToPromptSlashCommand = /* @__PURE__ */ __name((message) => {
  const { parts } = message;
  if (parts.length < 1) {
    return false;
  }
...
const renderPrompt = /* @__PURE__ */ __name((turns) => {
  const content = [];
...
const renderTools = /* @__PURE__ */ __name((tools) => {
  const toolStrings = [...tools].map((tool) => {
    return `'${tool}'`;
  });
  return `tools: [${toolStrings.join(", ")}]`;
}, "renderTools");
const renderHeader = /* @__PURE__ */ __name((tools) => {
  if (tools.size === 0) {
    return "";
  }
  return [
...
const runSaveToPromptAction = /* @__PURE__ */ __name(async (options, commandService) => {
  return await commandService.executeCommand(SAVE_TO_PROMPT_ACTION_ID, options);
}, "runSaveToPromptAction");
const registerSaveToPromptActions = /* @__PURE__ */ __name(() => {
...
//# sourceMappingURL=chatSaveToPromptAction.js.map
