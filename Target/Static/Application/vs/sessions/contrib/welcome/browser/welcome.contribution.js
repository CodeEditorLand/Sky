...
let SessionsWelcomeOverlay = class SessionsWelcomeOverlay2 extends Disposable {
  static {
    __name(this, "SessionsWelcomeOverlay");
  }
  constructor(container, chatEntitlementService, commandService, extensionService, logService) {
    ...
    append(header, $("h2", void 0, localize("welcomeTitle", "Sign in to use Sessions")));
    append(header, $("p.sessions-welcome-subtitle", void 0, localize("welcomeSubtitle", "Agent-powered development")));
  ...
  async _runSetup(button, spinner, error) {
    ...
    append(spinner, $("span", void 0, localize("sessions.settingUp", "Setting up\u2026")));
    ...
    try {
      const success = await this.commandService.executeCommand(CHAT_SETUP_SUPPORT_ANONYMOUS_ACTION_ID, {
        ...
        dialogTitle: this.chatEntitlementService.anonymous ? localize("sessions.startUsingSessions", "Start using Sessions") : localize("sessions.signinRequired", "Sign in to use Sessions"),
        dialogHideSkip: true
      });
      if (success) {
        spinner.textContent = "";
        spinner.appendChild(renderIcon(Codicon.loading));
        append(spinner, $("span", void 0, localize("sessions.restarting", "Completing setup\u2026")));
        this.logService.info("[sessions welcome] Restarting extension host after setup completion");
        const stopped = await this.extensionService.stopExtensionHosts(localize("sessionsWelcome.restart", "Completing sessions setup"));
        if (stopped) {
...
let SessionsWelcomeContribution = class SessionsWelcomeContribution2 extends Disposable {
  static {
    __name(this, "SessionsWelcomeContribution");
  }
  static {
    this.ID = "workbench.contrib.sessionsWelcome";
  }
  constructor(chatEntitlementService, layoutService, instantiationService, productService, storageService, contextKeyService) {
    super();
    this.chatEntitlementService = chatEntitlementService;
    ...
    const isFirstLaunch = !this.storageService.getBoolean(WELCOME_COMPLETE_KEY, -1, false);
    if (isFirstLaunch) {
      this.showOverlay();
    } else {
...
registerWorkbenchContribution2(
  SessionsWelcomeContribution.ID,
  SessionsWelcomeContribution,
  2
