...
let CompositeBar = class CompositeBar2 extends Widget {
  ...
  updateCompositeSwitcher(donotTrigger) {
    ...
    if (totalComposites > compositesToShow.length && !this.compositeOverflowAction) {
      ...
      this.compositeOverflowActionViewItem = this._register(this.instantiationService.createInstance(CompositeOverflowActivityActionViewItem, this.compositeOverflowAction, () => this.getOverflowingComposites(), () => this.model.activeItem ? this.model.activeItem.id : void 0, (compositeId) => {
        const item = this.model.findItem(compositeId);
