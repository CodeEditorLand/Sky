...
class ExtHostNotebookController {
    ...
    try {
      ...
      if (!this._extHostFileSystem.value.isWritableFileSystem(uri.scheme)) {
        throw new files.FileOperationError(
          localize("err.readonly", "Unable to modify read-only file '{0}'", this._resourceForError(uri)),
          6
  ...
  _createExtHostEditor(document, editorId, data) {
    ...
    const editor = new ExtHostNotebookEditor(editorId, this._notebookEditorsProxy, document, data.visibleRanges.map(typeConverters.NotebookRange.to), data.selections.map(typeConverters.NotebookRange.to), typeof data.viewColumn === "number" ? typeConverters.ViewColumn.to(data.viewColumn) : void 0, data.viewType);
