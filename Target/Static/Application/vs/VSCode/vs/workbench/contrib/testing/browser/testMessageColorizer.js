...
const renderTestMessageAsText = /* @__PURE__ */ __name((tm) => typeof tm === "string" ? removeAnsiEscapeCodes(tm) : renderAsPlaintext(tm), "renderTestMessageAsText");
    ...
    for (const part of forAnsiStringParts(message)) {
      if (part.isCode) {
        ...
        if (n === 0) {
          ...
          cls = cls.filter(
            (c) => c !== "tstm-ansidec-1" && c !== "tstm-ansidec-3"
            /* Classes.Italic */
          );
        } else if (n === 23) {
          cls = cls.filter(
