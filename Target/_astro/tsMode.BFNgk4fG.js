import { t as L, m as O } from "./editor.main.COpP3lv-.js";
		...
		if ((t && typeof t == "object") || typeof t == "function")
			for (let n of M(t))
				!R.call(e, n) &&
					...
					I(e, n, {
						get: () => t[n],
						enumerable: !(o = N(t, n)) || o.enumerable,
					});
		return e;
	},
	E = (e, t, i) => (K(e, t, "default"), i),
	a = {};
E(a, O);
var H = class {
		constructor(e, t) {
					...
					(this._client = (async () => (
						(this._worker = a.editor.createWebWorker({
							...
							createData: {
								...
								customWorkerPath:
									this._defaults.workerOptions
										.customWorkerPath,
								inlayHintsOptions:
									this._defaults.inlayHintsOptions,
							},
						})),
		...
		async getLanguageServiceWorker(...e) {
			...
			return (
				this._worker && (await this._worker.withSyncedResources(e)), t
			);
		}
	},
	r = {};
...
function C(e, t, i = 0) {
	...
	if (((o += e.messageText), i++, e.next))
		for (const n of e.next) o += C(n, t, i);
	return o;
}
function _(e) {
		...
		_convertRelatedInformation(e, t) {
			...
			return (
				t.forEach((o) => {
					...
					if (
						(o.file &&
							(n = this._libFiles.getOrCreateModel(
								o.file.fileName,
							)),
						!n)
					)
	...
	j = class x extends m {
				...
				: {
						suggestions: d.entries.map((f) => {
							...
							if (f.replacementSpan) {
								const k = t.getPositionAt(
										f.replacementSpan.start,
									),
									v = t.getPositionAt(
										f.replacementSpan.start +
											f.replacementSpan.length,
									);
								w = new a.Range(
									k.lineNumber,
									k.column,
							...
							return (
								f.kindModifiers !== void 0 &&
									f.kindModifiers.indexOf("deprecated") !==
										-1 &&
									S.push(
										a.languages.CompletionItemTag
											.Deprecated,
									),
								{
									uri: s,
		...
		async resolveCompletionItem(t, i) {
			const o = t,
				...
				c = o.position,
				u = o.offset,
				l = await (await this._worker(n)).getCompletionEntryDetails(
					n.toString(),
					u,
					o.label,
				);
...
var B = class T extends m {
		...
		async provideSignatureHelp(t, i, o, n) {
			...
			return (
				l.items.forEach((d) => {
					const h = { label: "", parameters: [] };
					(h.documentation = { value: _(d.documentation) }),
						(h.label += _(d.prefixDisplayParts)),
						d.parameters.forEach((f, w, S) => {
							const k = _(f.displayParts),
								v = {
									label: k,
									documentation: {
							...
							(h.label += k),
								h.parameters.push(v),
								w < S.length - 1 &&
									(h.label += _(d.separatorDisplayParts));
						}),
						(h.label += _(d.suffixDisplayParts)),
						g.signatures.push(h);
				}),
				{ value: g, dispose() {} }
			);
			...
			return !d || e.isDisposed()
				...
				: {
						actions: d
							.filter(
								(f) =>
									f.changes.filter((w) => w.isNewFile)
										.length === 0,
							)
							.map((f) =>
	...
	Z = class extends m {
		async provideInlayHints(e, t, i) {
			...
			return e.isDisposed()
				? null
				: {
						hints: (await s.provideInlayHints(n, c, u)).map(
							(d) => ({
								...d,
								label: d.text,
...
function P(e, t) {
	...
	function u() {
		...
		ee(i),
			s.completionItems &&
				i.push(a.languages.registerCompletionItemProvider(t, new j(n))),
			s.signatureHelp &&
				i.push(a.languages.registerSignatureHelpProvider(t, new B(n))),
			s.hovers && i.push(a.languages.registerHoverProvider(t, new U(n))),
			s.documentHighlights &&
				i.push(
					a.languages.registerDocumentHighlightProvider(t, new $(n)),
				),
			s.definitions &&
				i.push(a.languages.registerDefinitionProvider(t, new z(c, n))),
			s.references &&
				i.push(a.languages.registerReferenceProvider(t, new G(c, n))),
			s.documentSymbols &&
				i.push(a.languages.registerDocumentSymbolProvider(t, new J(n))),
			s.rename &&
				i.push(a.languages.registerRenameProvider(t, new Y(c, n))),
			...
			s.codeActions &&
				i.push(a.languages.registerCodeActionProvider(t, new X(n))),
			s.inlayHints &&
				i.push(a.languages.registerInlayHintsProvider(t, new Z(n))),
...
//# sourceMappingURL=tsMode.BFNgk4fG.js.map
