...
let FolderPicker = class FolderPicker2 extends Disposable {
  ...
  constructor(actionWidgetService, storageService, workspaceContextService, fileDialogService, commandService) {
    super();
  ...
  _buildItems(currentFolderUri) {
    ...
    for (const folder of dedupedFolders) {
      items.push({
        ...
        onRemove: /* @__PURE__ */ __name(() => this._removeFolder(folder.uri), "onRemove")
      });
    }
    if (items.length > 0) {
      items.push({
