...
suite("SessionEditorComments", () => {
  ...
  __name(reviewState, "reviewState");
  ...
  test("filters resource comments and detects authored feedback presence", () => {
    ...
    assert.deepStrictEqual(getResourceEditorComments(fileA, comments).map((comment) => comment.source), [
      "agentFeedback"
      /* SessionEditorCommentSource.AgentFeedback */
    ]);
    assert.deepStrictEqual(getResourceEditorComments(fileB, comments).map((comment) => comment.source), [
      "codeReview"
      /* SessionEditorCommentSource.CodeReview */
    ]);
  });
