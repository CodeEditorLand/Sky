...
let ChatListItemRenderer = class ChatListItemRenderer2 extends Disposable {
  ...
  renderConfirmationAction(element, templateData) {
    ...
    if (element.confirmation) {
      ...
      const updateCheckmarks = /* @__PURE__ */ __name(() => templateData.detail.classList.toggle("show-checkmarks", !!this.configService.getValue(
  ...
  shouldShowWorkingProgress(element, partsToRender, moreContentAvailable, templateData) {
    ...
    if (lastPart && (lastPart.kind === "toolInvocation" || lastPart.kind === "toolInvocationSerialized")) {
      ...
      if (collapsedToolsMode !== CollapsedToolsDisplayMode.Off && this.shouldPinPart(lastPart, isResponseVM(element) ? element : void 0)) {
        return false;
      }
    }
  ...
  renderMarkdown(markdown, templateData, context) {
    ...
    if ((!this.hasEditCodeblockUri(markdown) || isFinalAnswerPart) && !isBlankMarkdown && !hasPendingEditCodeblock) {
      this.finalizeCurrentThinkingPart(context, templateData);
    }
