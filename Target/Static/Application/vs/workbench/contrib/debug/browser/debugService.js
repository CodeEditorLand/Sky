    ...
    if (configByProviders && configByProviders.type) {
      try {
        ...
        if (!dbg || configByProviders.request !== "attach" && configByProviders.request !== "launch") {
          ...
          const actionList = [];
          actionList.push(new Action("installAdditionalDebuggers", nls.localize({ key: "installAdditionalDebuggers", comment: ['Placeholder is the debug type, so for example "node", "python"'] }, "Install {0} Extension", resolvedConfig.type), void 0, true, async () => this.commandService.executeCommand("debug.installAdditionalDebuggers", resolvedConfig?.type)));
