...
let McpService = class McpService2 extends Disposable {
  ...
  async _autostart(autoStartConfig, state, token) {
    ...
    const candidates = this.servers.get().filter((s) => s.connectionState.get().state !== 3 && isContributionEnabled(s.enablement.get()));
  ...
  updateCollectedServers() {
    ...
    for (const def of nextDefinitions) {
      const object = this._instantiationService.createInstance(McpServer, def.collectionDefinition, def.serverDefinition, def.serverDefinition.roots, !!def.collectionDefinition.lazy, def.collectionDefinition.scope === 1 ? this.workspaceCache : this.userCache, def.toolPrefix, this.enablementModel);
