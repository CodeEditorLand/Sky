...
suite("TreeSitterCommandParser", () => {
  ...
  suite("extractSubCommands", () => {
    suite("bash", () => {
      async function t(commandLine, expectedCommands) {
        const result = await parser.extractSubCommands("bash", commandLine);
      ...
      suite("edge cases", () => {
        ...
        test("very long command lines", () => t("echo " + "a".repeat(1e4) + " && ls", ["echo " + "a".repeat(1e4), "ls"]));
        test("special characters", () => t('echo "\u03C0\u03BB\u03B1\u03C2\u03B5 \u6D4B\u8BD5 \u{1F680}" && ls', ['echo "\u03C0\u03BB\u03B1\u03C2\u03B5 \u6D4B\u8BD5 \u{1F680}"', "ls"]));
    ...
    suite("pwsh", () => {
      async function t(commandLine, expectedCommands) {
        const result = await parser.extractSubCommands("powershell", commandLine);
        deepStrictEqual(result, expectedCommands);
      }
      __name(t, "t");
  ...
  suite("extractPwshDoubleAmpersandChainOperators", () => {
    ...
    suite("edge cases", () => {
      ...
      test("&& with unicode", () => t('Write-Host "\u6D4B\u8BD5" && Write-Host "\u{1F680}"', ["&&"]));
      test("very long command with &&", () => t('Write-Host "' + "a".repeat(1e3) + '" && Get-Date', ["&&"]));
  ...
  suite("getFileWrites", () => {
    suite("bash", () => {
      async function t(commandLine, expectedFiles) {
        const actualFiles = await parser.getFileWrites("bash", commandLine);
    ...
    suite("pwsh", () => {
      async function t(commandLine, expectedFiles) {
        const actualFiles = await parser.getFileWrites("powershell", commandLine);
        deepStrictEqual(actualFiles, expectedFiles);
      }
      __name(t, "t");
...
//# sourceMappingURL=treeSitterCommandParser.test.js.map
