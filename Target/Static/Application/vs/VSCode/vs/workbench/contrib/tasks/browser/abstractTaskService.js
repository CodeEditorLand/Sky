...
let AbstractTaskService = class AbstractTaskService2 extends Disposable {
  ...
  async _handleLongRunningTaskCompletion(event, durationMs) {
    const notificationThreshold = this._configurationService.getValue(
  ...
  async _activateTaskProviders(type) {
    ...
    const result = await raceTimeout(Promise.all(this._getActivationEvents(type).map((activationEvent) => this._extensionService.activateByEvent(activationEvent))), 5e3, () => this._logService.warn("Timed out activating extensions for task providers"));
    if (result) {
...
`);
  ...
  async _computeWorkspaceTasks(runSource = 1) {
    ...
    const hasAnyTasks = Array.from(result.values()).some((folderResult) => folderResult.set?.tasks && folderResult.set.tasks.length > 0 || folderResult.configurations?.byIdentifier && Object.keys(folderResult.configurations.byIdentifier).length > 0);
