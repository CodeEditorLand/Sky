...
import { Location as c, getCurrentUrl as L } from "./Swup.modern.BemQoQWS.js";
function k() {
	return (
		(k = Object.assign
			? Object.assign.bind()
...
const E = window.requestIdleCallback || ((u) => setTimeout(u, 1)),
...
class A extends V {
	constructor(e = {}) {
		...
		super(),
			...
			(this.onMouseEnter = async function (t) {
				if (t.target !== t.delegateTarget || !P()) return;
				const r = t.delegateTarget;
				if (!b(r)) return;
				const { url: a, hash: n } = c.fromElement(r),
					l = o.swup.createVisit({ to: a, hash: n, el: r, event: t });
				o.swup.hooks.callSync("link:hover", l, { el: r, event: t }),
					o.preload(r, { priority: !0 });
			}),
			(this.onTouchStart = (t) => {
				if (P()) return;
				const r = t.delegateTarget;
				b(r) && this.preload(r, { priority: !0 });
			}),
			(this.onFocus = (t) => {
				const r = t.delegateTarget;
				b(r) && this.preload(r, { priority: !0 });
		...
		const { preloadVisibleLinks: s } = e,
			i = (function (t, r) {
				...
				for (n = 0; n < p.length; n++)
					r.indexOf((a = p[n])) >= 0 || (l[a] = t[a]);
				return l;
			})(e, O);
		(this.options = k({}, this.defaults, i)),
			typeof s == "object"
	...
	mount() {
		...
		e.options.cache
			? (e.hooks.create("page:preload"),
				...
				this.options.preloadInitialPage && this.preload(L()))
			: console.warn(
	...
	async preload(e, o = {}) {
		var s;
		let i, t;
		const r = (s = o.priority) != null && s;
		if (Array.isArray(e)) return Promise.all(e.map((n) => this.preload(n)));
		if (b(e)) (t = e), ({ href: i } = c.fromElement(e));
		else {
		...
		if (!this.shouldPreload(i, { el: t })) return;
		...
		return this.preloadPromises.set(i, a), a;
	}
	preloadLinks() {
		E(() => {
			Array.from(
		...
		(this.preloadObserver = (function ({
			...
			const l = new Map(),
				p = new IntersectionObserver(
					(d) => {
						d.forEach((h) => {
							h.isIntersecting ? f(h.target) : v(h.target);
						});
					},
					{ threshold: i },
				),
				f = (d) => {
					var h;
					const { href: m } = c.fromElement(d),
						y = (h = l.get(m)) != null ? h : new Set();
					l.set(m, y),
						y.add(d),
						setTimeout(() => {
							const w = l.get(m);
							w != null &&
								w.size &&
								(a(d), p.unobserve(d), w.delete(d));
						}, t);
				},
				v = (d) => {
					var h;
					const { href: m } = c.fromElement(d);
					(h = l.get(m)) == null || h.delete(d);
				},
				g = () => {
					E(() => {
						const d = r.map((h) => `${h} a[*|href]`).join(", ");
						Array.from(document.querySelectorAll(d))
							.filter((h) => n(h))
							.forEach((h) => p.observe(h));
					});
				};
			return {
				start: () => g(),
			...
			filter: (i) => {
				if (
					...
					return !1;
				const { href: t } = c.fromElement(i);
				return this.shouldPreload(t, { el: i });
			},
		})),
	...
	shouldPreload(e, { el: o } = {}) {
		const { url: s, href: i } = c.fromUrl(e);
		return !(
			!(function () {
			...
			this.swup.shouldIgnoreVisit(i, { el: o }) ||
			(o && this.swup.resolveUrl(s) === this.swup.resolveUrl(L()))
		);
	}
	async performPreload(e) {
		...
		return await this.swup.hooks.call(
			...
			async function (r, a) {
				return (
					(a.page = await o.swup.fetchPage(e, { visit: r })), a.page
				);
			},
		);
	}
...
export { A as default };
//# sourceMappingURL=index.modern.CUPCPLSa.js.map
