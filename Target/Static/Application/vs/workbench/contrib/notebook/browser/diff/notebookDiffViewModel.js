...
class NotebookDiffViewModel extends Disposable {
  ...
  async createDiffViewModels(computedCellDiffs, metadataChanged) {
    ...
    const cellViewModels = await Promise.all(computedCellDiffs.map(async (diff) => {
      switch (diff.type) {
        case "delete": {
          return new SingleSideDiffElementViewModel(originalModel, modifiedModel, originalModel.cells[diff.originalCellIndex], void 0, "delete", this.eventDispatcher, initData, this.notebookService, this.configurationService, this.diffEditorHeightCalculator, diff.originalCellIndex);
        }
        case "insert": {
          return new SingleSideDiffElementViewModel(modifiedModel, originalModel, void 0, modifiedModel.cells[diff.modifiedCellIndex], "insert", this.eventDispatcher, initData, this.notebookService, this.configurationService, this.diffEditorHeightCalculator, diff.modifiedCellIndex);
        }
        case "modified": {
