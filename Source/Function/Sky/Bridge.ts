...
async function _InstallSkyBridgeOnce(): Promise<void> {
	...
	// Mountain emits this when an extension calls `editor.setDecorations(type, ranges)`.
	// We route each entry to ICodeEditorService → find editor by URI → `editor.setDecorations(key, ranges)`.
	// The payload is `{ batch: [{ decorationTypeKey, uri, rangesOrOptions }] }`.
	...
	// Wire into the workbench's ICodeEditorService events so Mountain (and
	// through it, Cocoon) always knows the current editor and selection.
	try {
		const Services = GetServices();
		const CodeEditorService = (Services as any)?.CodeEditorService;
	...
	// Mountain emits this when an extension calls `editor.revealRange(range, revealType)`.
	// Payload: `{ uri, range: {startLineNumber, startColumn, endLineNumber, endColumn}, revealType }`.
	// RevealType enum: 0=Simple, 1=Center, 2=CenterIfOutsideViewport, 3=NearTop, 4=NearTopIfOutsideViewport
		...
		for (const Ed of Editors) {
			try {
				...
				if (
					...
					continue;
				// Monaco range is 1-based; payload already in Mountain 1-based form
				const MonacoRange = {
					startLineNumber:
