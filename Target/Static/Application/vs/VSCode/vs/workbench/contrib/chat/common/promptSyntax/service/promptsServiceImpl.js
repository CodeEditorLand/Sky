...
let PromptsService = class PromptsService2 extends Disposable {
  ...
  async getPromptSlashCommands(token, sessionResource) {
    ...
    if (sessionResource) {
      ...
      void this.getPromptSlashCommandDiscoveryInfo(token).catch(() => void 0).then((discoveryInfo) => {
        const details = result.length === 1 ? localize("promptsService.resolvedSlashCommand", "Resolved {0} slash command in {1}ms", result.length, elapsed.toFixed(1)) : localize("promptsService.resolvedSlashCommands", "Resolved {0} slash commands in {1}ms", result.length, elapsed.toFixed(1));
  ...
  async getCustomAgents(token, sessionResource) {
    ...
    if (sessionResource) {
      const elapsed = sw.elapsed();
      void this.getAgentDiscoveryInfo(token).catch(() => void 0).then((discoveryInfo) => {
  ...
  getPromptLocationLabel(promptPath) {
    switch (promptPath.storage) {
      ...
      case PromptsStorage.internal:
        return localize("internal.builtIn", "Built-in");
      default:
  ...
  async findAgentSkills(token, sessionResource) {
    ...
    if (sessionResource) {
      const elapsed = sw.elapsed();
      void this.getSkillDiscoveryInfo(token).catch(() => void 0).then((discoveryInfo) => {
  ...
  async getHooks(token, sessionResource) {
    ...
    if (sessionResource) {
      const elapsed = sw.elapsed();
      void this.getHookDiscoveryInfo(token).catch(() => void 0).then((discoveryInfo) => {
  ...
  async getInstructionFiles(token, sessionResource) {
    ...
    if (sessionResource) {
      const elapsed = sw.elapsed();
      void this.getInstructionsDiscoveryInfo(token).catch(() => void 0).then((discoveryInfo) => {
  ...
  async getSkillDiscoveryInfo(token) {
    ...
    if (!useAgentSkills) {
      ...
      return { type: PromptsType.skill, files: files2, sourceFolders };
    }
