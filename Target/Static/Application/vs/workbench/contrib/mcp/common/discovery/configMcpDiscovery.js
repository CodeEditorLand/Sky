...
let ConfigMcpDiscovery = class ConfigMcpDiscovery2 extends Disposable {
  ...
  async sync() {
    ...
    for (const [index, src] of this.configSources.entries()) {
      ...
      const { isAbsolute, join, sep } = src.path.remoteAuthority && remoteEnv ? remoteEnv.os === 1 ? pathWin32 : pathPosix : isWindows ? pathWin32 : pathPosix;
      ...
      const nextDefinitions = Object.entries(value?.servers || {}).map(([name, value2]) => ({
        ...
        launch: "url" in value2 ? {
          ...
          cwd: value2.cwd ? !isAbsolute(value2.cwd) && !value2.cwd.startsWith("~") && !value2.cwd.startsWith("${") && src.path.workspaceFolder ? join(fsPathForRemote(src.path.workspaceFolder.uri), value2.cwd) : value2.cwd : src.path.workspaceFolder ? fsPathForRemote(src.path.workspaceFolder.uri) : void 0
        },
        roots: src.path.workspaceFolder ? [src.path.workspaceFolder.uri] : void 0,
        variableReplacement: {
