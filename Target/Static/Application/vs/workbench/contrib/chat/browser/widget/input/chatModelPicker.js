...
function createModelItem(action, model) {
  return {
    ...
    hover: model ? { content: getModelHoverContent(model) } : void 0
  };
}
...
function buildModelPickerItems(models, selectedModelId, recentModelIds, controlModels, currentVSCodeVersion, updateStateType, onSelect, manageSettingsUrl, canManageModels, commandService, chatEntitlementService) {
  const items = [];
  if (models.length === 0) {
...
function createUnavailableModelItem(id, entry, reason, manageSettingsUrl, updateStateType, section) {
  let description;
  if (reason === "upgrade") {
...
let ModelPickerWidget = class ModelPickerWidget2 extends Disposable {
  ...
  constructor(_delegate, _actionWidgetService, _commandService, _telemetryService, _languageModelsService, _productService, _entitlementService, _updateService) {
    super();
