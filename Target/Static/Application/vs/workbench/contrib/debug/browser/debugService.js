...
let DebugService = class DebugService2 {
    ...
    if (configByProviders && configByProviders.type) {
      try {
        ...
        if (options?.startedByUser && resolvedConfig && resolvedConfig.suppressMultipleSessionWarning !== true) {
          ...
          const existingSession = existingSessions.find((s) => s.configuration.name === resolvedConfig.name && s.configuration.type === resolvedConfig.type && s.configuration.request === resolvedConfig.request && s.root === workspace2);
          if (existingSession) {
        ...
        if (!dbg || configByProviders.request !== "attach" && configByProviders.request !== "launch") {
          ...
          actionList.push(toAction({
            ...
            enabled: true,
            run: /* @__PURE__ */ __name(async () => this.commandService.executeCommand("debug.installAdditionalDebuggers", resolvedConfig?.type), "run")
          }));
  ...
  async doCreateSession(sessionId, root, configuration, options, userConfirmedConcurrentSession = false) {
    ...
    if (!userConfirmedConcurrentSession && options?.startedByUser && this.model.getSessions().some((s) => s.configuration.name === configuration.resolved.name && s.configuration.type === configuration.resolved.type && s.configuration.request === configuration.resolved.request && s.root === root) && configuration.resolved.suppressMultipleSessionWarning !== true) {
  ...
  registerSessionListeners(session) {
    ...
    listenerDisposables.add(session.onDidEndAdapter(async (adapterExitEvent) => {
      ...
      if (this.configurationService.getValue("debug").closeReadonlyTabsOnEnd) {
        const editorsToClose = this.editorService.getEditors(
          1
  ...
  async showError(message, errorActions = [], promptLaunchJson = true) {
    const configureAction = toAction({ id: DEBUG_CONFIGURE_COMMAND_ID, label: DEBUG_CONFIGURE_LABEL, enabled: true, run: /* @__PURE__ */ __name(() => this.commandService.executeCommand(DEBUG_CONFIGURE_COMMAND_ID), "run") });
  ...
  async makeTriggeredBreakpointsMatchEnablement(enable, breakpoint) {
    ...
    await Promise.all(this.model.getBreakpoints().filter((bp) => bp.triggeredBy === breakpoint.getId() && bp.enabled !== enable).map((bp) => this.enableOrDisableBreakpoints(enable, bp)));
  }
  ...
  onFileChanges(fileChangesEvent) {
    const toRemove = this.model.getBreakpoints().filter((bp) => fileChangesEvent.contains(
      bp.originalUri,
      2
