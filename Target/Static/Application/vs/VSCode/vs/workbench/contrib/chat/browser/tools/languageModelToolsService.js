...
let LanguageModelToolsService = class LanguageModelToolsService2 extends Disposable {
  ...
  async invokeTool(dto, countTokens, token) {
    ...
    try {
      ...
      const afterExecuteState = await toolInvocation?.didExecuteTool(toolResult, void 0, () => this.shouldAutoConfirmPostExecution(tool.data.id, tool.data.runsInWorkspace, tool.data.source, dto.parameters, dto.context?.sessionResource, dto.chatRequestId));
...
${msgText}`),
  ...
  async shouldAutoConfirm(toolId, runsInWorkspace, source, parameters, chatSessionResource, chatRequestId) {
    ...
    if (chatSessionResource && !this._isAutoApprovePolicyRestricted()) {
      ...
      const request = model?.getRequests().at(-1);
      if (isAutoApproveLevel(request?.modeInfo?.permissionLevel) || this._isSessionLiveAutoApproveLevel(chatSessionResource)) {
        return { type: 1, reason: "auto-approve-all" };
      }
    }
  ...
  async _doCheckGlobalAutoApprove() {
    ...
    try {
      ...
      store.add(cts);
      store.add(this._storageService.onDidChangeValue(-1, "chat.tools.global.autoApprove.optIn", store)(() => {
        if (this._storageService.getBoolean("chat.tools.global.autoApprove.optIn", -1, false)) {
          cts.cancel();
        }
      }));
