...
class ListView {
  ...
  insertItemInDOM(index, row) {
    ...
    const toAriaState = /* @__PURE__ */ __name((value) => value === "mixed" ? "mixed" : String(!!value), "toAriaState");
    if (typeof checked === "boolean" || checked === "mixed") {
      item.row.domNode.setAttribute("aria-checked", toAriaState(checked));
    } else if (checked) {
      const update = /* @__PURE__ */ __name((value) => item.row.domNode.setAttribute("aria-checked", toAriaState(value)), "update");
