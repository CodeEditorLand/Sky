import { i as q } from "./index.D6rU_tt3.js";
...
const K = () => {
		...
		if (t?.closest(`[${y}]`)) {
			if (
				...
				const e = t.selectionStart,
					n = t.selectionEnd;
				return () => E({ activeElement: t, start: e, end: n });
			}
			return () => E({ activeElement: t });
		} else return () => E({ activeElement: null });
	},
	E = ({ activeElement: t, start: e, end: n }) => {
		t &&
			(t.focus(),
			(t instanceof HTMLInputElement ||
				t instanceof HTMLTextAreaElement) &&
	...
	J = "astro:before-preparation",
	Q = "astro:after-preparation",
	Z = "astro:before-swap",
	tt = "astro:after-swap",
	et = (t) => document.dispatchEvent(new Event(t));
class H extends Event {
	...
	newDocument;
	signal;
	constructor(e, n, o, r, s, u, a, c, d, l) {
		super(e, n),
			(this.from = o),
			(this.to = r),
			(this.direction = s),
...
class nt extends H {
	...
	loader;
	constructor(e, n, o, r, s, u, a, c, d, l) {
		super(J, { cancelable: !0 }, e, n, o, r, s, u, a, c),
			(this.formData = d),
...
async function rt(t, e, n, o, r, s, u, a, c) {
	const d = new nt(t, e, n, o, r, s, window.document, u, a, c);
	return (
...
const st = history.pushState.bind(history),
...
const X = (t) => document.dispatchEvent(new Event(t)),
	...
	L = "data-astro-transition-persist",
	P = "data-astro-transition",
...
const C = (t, e, n, o, r) => {
	...
	if (t.href !== location.href && !r)
		if (n.history === "replace") {
			...
			st({ ...n.state, index: ++g, scrollX: 0, scrollY: 0 }, "", t.href);
	if (
		((document.title = u),
		(v = t),
	...
	for (const n of t.querySelectorAll("head link[rel=stylesheet]"))
		if (
			!document.querySelector(
				`[${L}="${n.getAttribute(L)}"], link[rel=stylesheet][href="${n.getAttribute("href")}"]`,
			)
		) {
...
async function I(t, e, n, o, r) {
	async function s(c) {
		...
		document.documentElement.setAttribute(S, c);
		const w = document
			.getAnimations()
			.filter((h) => !l.includes(h) && !d(h));
...
async function $(t, e, n, o, r) {
	...
	async function c(i) {
		...
		const A = ut(i.newDocument);
		A.length && !i.signal.aborted && (await Promise.all(A));
	}
	async function d() {
	...
	if ((document.documentElement.setAttribute(P, a.direction), x))
		l.viewTransition = document.startViewTransition(
			async () => await I(a, o, l, r),
		);
	else {
		const i = (async () => {
			await Promise.resolve(), await I(a, o, l, r, _());
		})();
		l.viewTransition = {
			updateCallbackDone: i,
			ready: i,
			finished: new Promise((w) => (l.viewTransitionFinished = w)),
			skipTransition: () => {
...
async function M(t, e) {
	await $("forward", v, new URL(t, location.href), e ?? {});
}
function ft(t) {
	if (!D() && t.state) {
		location.reload();
	...
	const e = history.state,
		n = e.index,
		o = n > g ? "forward" : "back";
	(g = n), $(o, v, new URL(location.href), {}, e);
}
const N = () => {
	history.state &&
...
(x || mt() !== "none") &&
	...
	document.addEventListener("submit", (t) => {
		...
		t.preventDefault(), M(a, d);
	}),
	q({ prefetchAll: !0 }));
//# sourceMappingURL=ViewTransitions.astro_astro_type_script_index_0_lang.Bvzvg9jW.js.map
