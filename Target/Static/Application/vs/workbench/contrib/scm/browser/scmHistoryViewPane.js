...
let HistoryItemRenderer = class HistoryItemRenderer2 {
  ...
  _getHoverOptions() {
    if (this._viewContainerLocation === 1) {
      return {
        hoverOptions: {
...
class SCMHistoryTreeDragAndDrop {
  static {
    __name(this, "SCMHistoryTreeDragAndDrop");
  }
  getDragURI(element) {
...
let SCMHistoryViewModel = class SCMHistoryViewModel2 extends Disposable {
  ...
  async getHistoryItems() {
    ...
    if (!state || state.loadMore !== false) {
      ...
      const viewModels = toISCMHistoryItemViewModelArray(historyItems, colorMap, historyProvider.historyItemRef.get(), historyProvider.historyItemRemoteRef.get(), historyProvider.historyItemBaseRef.get(), addIncomingChangesNode, addOutgoingChangesNode, mergeBase).map((historyItemViewModel) => ({
        repository,
    ...
    if (isSCMHistoryItemViewModelTreeElement(element)) {
      ...
      if (historyItemRefMenuItems.length > 0 && element.historyItemViewModel.historyItem.references?.length) {
        ...
        for (const ref of element.historyItemViewModel.historyItem.references) {
          ...
          const menuActions2 = this._menuService.getMenuActions(MenuId.SCMHistoryItemRefContext, contextKeyService2);
      ...
      this.contextMenuService.showContextMenu({
        ...
        getActions: /* @__PURE__ */ __name(() => getFlatContextMenuActions(menuActions), "getActions"),
      ...
      this.contextMenuService.showContextMenu({
        ...
        getAnchor: /* @__PURE__ */ __name(() => e.anchor, "getAnchor"),
        getActions: /* @__PURE__ */ __name(() => getFlatContextMenuActions(menuActions), "getActions"),
        getActionsContext: /* @__PURE__ */ __name(() => element.historyItemChange, "getActionsContext")
      });
    }
  }
