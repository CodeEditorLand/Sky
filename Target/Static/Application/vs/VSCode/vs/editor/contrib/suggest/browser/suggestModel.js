...
let SuggestModel = SuggestModel_1 = class SuggestModel2 {
    ...
    this._triggerQuickSuggest.cancelAndSet(() => {
      ...
      if (!QuickSuggestionsOptions.isAllOn(config)) {
        ...
        if (value === "offWhenInlineCompletions") {
          waitForInlineCompletions = this._languageFeaturesService.inlineCompletionsProvider.has(model) && this._editor.getOption(
            71
            /* EditorOption.inlineSuggest */
          ).enabled;
        }
  ...
  _waitForInlineCompletionsAndTrigger(initialModel, initialPosition) {
    ...
    const triggerAndCleanUp = /* @__PURE__ */ __name((doTrigger) => {
      store.dispose();
