...
interface ISandboxConfiguration {
	...
	execPath: string; // Filesystem path
	zoomLevel?: number;
	homeDir: string; // URI string
	tmpDir: string; // URI string
	userDataDir: string; // URI string
	backupPath?: string; // URI string
	...
	VSCODE_CWD?: string; // Filesystem path
	resourcesPath: string; // Filesystem path (this is what process.resourcesPath should point to)
	[key: string]: any; // Allow other fields VS Code might expect
}
...
function getFallbackSandboxConfiguration(): ISandboxConfiguration {
	...
	const fallbackAppRoot = fallbackAppRootFromGlobal
		? fallbackAppRootFromGlobal.startsWith("file://")
			...
			: `file://${fallbackAppRootFromGlobal}` // Ensure it's a file URI if not already
		: `${fallbackOrigin}/Static/Application/`; // Absolute fallback
	...
	let derivedArch: ProcessShim["arch"] = "x64"; // Default
	if (
		typeof navigator !== "undefined" &&
	...
	const fallbackResourcesPath = "/app/fallback_resources"; // Must be a direct fs path

	return {
		windowId: Math.floor(Math.random() * 100000) + 1, // Ensure non-zero
		machineId: fallbackMachineId,
		sessionId: fallbackSessionId,
		logLevel: LogLevel.Info,
		userEnv: { FALLBACK_MODE: "true" },
		appRoot: fallbackAppRoot, // URI
		...
		execPath: "/app/FIDDEE_fallback_executable", // Filesystem path
		homeDir: fallbackHomeDir, // URI
		tmpDir: `file:///tmp/fallback_${generateUuid()}`, // URI
		userDataDir: `file:///app/user_data_fallback_${generateUuid()}`, // URI
		backupPath: `file:///app/backup_fallback_${generateUuid()}`, // URI
		nls: {
			messages: {},
			language:
				(typeof navigator !== "undefined"
		...
		resourcesPath: fallbackResourcesPath, // Direct filesystem path
		VSCODE_CWD: "/app/fallback_cwd", // Filesystem path
		zoomLevel: 0,
	...
	resolveConfiguration: (): Promise<ISandboxConfiguration> => {
		...
		resolveConfigurationPromise = invoke(
			...
			.then((configDataFromMountain: unknown) => {
				...
				let fsResourcesPath = "/app/resources_placeholder"; // Default placeholder
				try {
					if (resolvedConfigurationCache.resourcesPath) {
						fsResourcesPath =
							resolvedConfigurationCache.resourcesPath.startsWith(
								...
								: resolvedConfigurationCache.resourcesPath; // Assume it's already a fsPath if not file URI
					} else if (
						...
						fsResourcesPath = URI.file(
							...
							path:
								URI.parse(resolvedConfigurationCache.appRoot)
									.fsPath + "/resources",
						}).fsPath; // Example: appRoot/resources
			...
			.catch((error: any) => {
				...
				resolvedConfigurationCache = getFallbackSandboxConfiguration(); // Ensure fallback is set on error too
				...
				if (resolvedConfigurationCache.resourcesPath)
					(processShimInstance as any).resourcesPath =
						resolvedConfigurationCache.resourcesPath; // Fallback should provide fsPath
				...
				if (document.body) {
					...
					document.body.prepend(errorDiv.firstChild!); // Prepend to avoid overwriting everything if workbench somehow loads
				} else {
					document.write(errorMessageHtml); // Last resort
				}
				return resolvedConfigurationCache; // Return fallback
			});
		return resolveConfigurationPromise;
	},
};
