...
let WebExtensionsScannerService = class WebExtensionsScannerService2 extends Disposable {
  static {
  ...
  async readSystemExtensions() {
    ...
    const cachedSystemExtensions = await Promise.all((await this.readSystemExtensionsCache()).map((e) => this.toScannedExtension(
      e,
      true,
      0
  ...
  async getCustomBuiltinExtensionsFromGallery(scanOptions) {
    ...
    try {
      ...
      const useCache = this.storageService.get("additionalBuiltinExtensions", -1, "{}") === cacheValue;
  ...
  async toWebExtensionFromGallery(galleryExtension, metadata) {
    const extensionLocation = await this.extensionResourceLoaderService.getExtensionGalleryResourceURL({
      ...
      targetPlatform: galleryExtension.properties.targetPlatform === "web" ? "web" : void 0
    }, "extension");
    if (!extensionLocation) {
      throw new Error("No extension gallery service configured.");
    }
    return this.toWebExtensionFromExtensionGalleryResource(extensionLocation, galleryExtension.identifier, galleryExtension.assets.readme ? URI.parse(galleryExtension.assets.readme.uri) : void 0, galleryExtension.assets.changelog ? URI.parse(galleryExtension.assets.changelog.uri) : void 0, metadata);
  }
  async toWebExtensionFromExtensionGalleryResource(extensionLocation, identifier, readmeUri, changelogUri, metadata) {
    ...
    return this.toWebExtension(extensionLocation, identifier, void 0, packageNLSResources, fallbackPackageNLSResource ? URI.parse(fallbackPackageNLSResource) : null, readmeUri, changelogUri, metadata);
  }
  ...
  async toScannedExtension(webExtension, isBuiltin, type = 1) {
    const validations = [];
