...
import { queryAll as u } from "./Swup.modern.BemQoQWS.js";
function a() {
	return (
		(a = Object.assign
			? Object.assign.bind()
...
class g {
	constructor(o) {
		(this._raf = null),
			...
			o && o.friction && (this.options.friction = 1 - o.friction),
...
class S extends p {
	constructor(o = {}) {
		super(),
			...
			(this.getAnchorElement = (t = "") =>
				typeof this.options.getAnchorElement == "function"
			...
			(this.handleScrollToTop = (t) => {
				var s, i;
				return (
					(s = (i = this.swup).scrollTo) == null ||
						s.call(i, 0, t.scroll.animate),
					!0
				);
			}),
			...
			(this.handleScrollToAnchor = (t, { hash: s }) =>
				this.maybeScrollToAnchor(s, t.scroll.animate)),
			...
			(this.onVisitStart = (t) => {
				...
				const i = (s = t.scroll.target) != null ? s : t.to.hash;
				t.scroll.animate &&
			...
			(this.doScrollingBetweenPages = (t) => {
				...
				const e = (s = t.scroll.target) != null ? s : t.to.hash;
				if (
					(e && this.maybeScrollToAnchor(e, t.scroll.animate)) ||
					!t.scroll.reset
				)
					return;
				const r = this.getCachedScrollPositions(t.to.url),
					l =
						(r == null || (i = r.window) == null
							? void 0
							: i.top) || 0;
				requestAnimationFrame(() => {
					var h, d;
					return (h = (d = this.swup).scrollTo) == null
						? void 0
						: h.call(d, l, t.scroll.animate);
				}),
					(t.scroll.scrolledToContent = !0);
			}),
			(this.maybeResetScrollPositions = (t) => {
				const { popstate: s } = t.history,
					{ url: i } = t.to,
					{ el: e } = t.trigger;
				s ||
					(e && !this.options.shouldResetScrollPosition(e)) ||
					this.resetScrollPositions(i);
			}),
			(this.options = c({}, this.defaults, o));
	}
	mount() {
		const o = this.swup;
		o.hooks.create("scroll:start"), o.hooks.create("scroll:end");
		const t = this.swup.createVisit({ to: this.swup.currentPageUrl });
		(this.scrl = new g({
			onStart: () => o.hooks.callSync("scroll:start", t, void 0),
			onEnd: () => o.hooks.callSync("scroll:end", t, void 0),
			onCancel: () => o.hooks.callSync("scroll:end", t, void 0),
			friction: this.options.scrollFriction,
			acceleration: this.options.scrollAcceleration,
		})),
			(o.scrollTo = (s, i = !0) => {
				i
					? this.scrl.scrollTo(s)
					: (o.hooks.callSync("scroll:start", t, void 0),
						window.scrollTo(0, s),
						o.hooks.callSync("scroll:end", t, void 0));
			}),
	...
	shouldAnimate(o) {
		return typeof this.options.animateScroll == "boolean"
			? this.options.animateScroll
			: this.options.animateScroll[o];
	}
	maybeScrollToAnchor(o, t = !1) {
		var s, i;
		if (!o) return !1;
		const e = this.getAnchorElement(o);
		if (!e) return console.warn(`Anchor target ${o} not found`), !1;
		if (!(e instanceof Element))
			return console.warn(`Anchor target ${o} is not a DOM node`), !1;
		const { top: r } = e.getBoundingClientRect(),
			l = r + window.scrollY - this.getOffset(e);
		return (s = (i = this.swup).scrollTo) == null || s.call(i, l, t), !0;
	}
	cacheScrollPositions(o) {
		const t = this.swup.resolveUrl(o),
			s = u(this.options.scrollContainers).map((e) => ({
				top: e.scrollTop,
				left: e.scrollLeft,
			})),
			i = {
		...
		this.cachedScrollPositions[t] = i;
	}
	resetScrollPositions(o) {
		const t = this.swup.resolveUrl(o);
		delete this.cachedScrollPositions[t];
	}
	getCachedScrollPositions(o) {
		const t = this.swup.resolveUrl(o);
		return this.cachedScrollPositions[t];
	}
	restoreScrollContainers(o) {
		const t = this.getCachedScrollPositions(o);
		t &&
			t.containers.length !== 0 &&
			u(this.options.scrollContainers).forEach((s, i) => {
				const e = t.containers[i];
				e != null && ((s.scrollTop = e.top), (s.scrollLeft = e.left));
			});
	}
	updateScrollTarget() {
		var o;
		...
		let i = this.getAnchorElement(t);
		i instanceof HTMLBodyElement && (i = null),
			s !== i &&
				(s?.removeAttribute("data-swup-scroll-target"),
				(o = i) == null ||
					o.setAttribute("data-swup-scroll-target", ""));
	}
}
export { S as default };
//# sourceMappingURL=index.modern.ljjH5-f0.js.map
