...
let ChatInputPart = class ChatInputPart2 extends Disposable {
      ...
      if (sessionResource) {
        ...
        if (ctx) {
          ...
          if (typeof agentOption !== "undefined") {
            ...
            needsUpdate = isDefaultAgent ? mode.id !== ChatMode.Agent.id : mode.label.read(void 0) !== agentId;
          }
          if (needsUpdate) {
  ...
  setInputModel(model, chatSessionIsEmpty) {
    ...
    if (chatSessionIsEmpty && !this.configurationService.getValue(ChatConfiguration.GlobalAutoApprove) && !isAutoApproveLevel(this._currentPermissionLevel.get())) {
      this._currentPermissionLevel.set(ChatPermissionLevel.Default, void 0);
    ...
    try {
      ...
      if (state?.selectedModel) {
        ...
        const syncResult = resolveModelFromSyncState(state.selectedModel, this._currentLanguageModel.get(), allModels, sessionType, {
          ...
          isInlineChatV2Enabled: !!this.configurationService.getValue(
            "inlineChat.enableV2"
  ...
  getModels() {
    ...
    return filterModelsForSession(models, this.getCurrentSessionType(), this.currentModeKind, this.location, !!this.configurationService.getValue(
      "inlineChat.enableV2"
      /* InlineChatConfigKeys.EnableV2 */
    ));
  }
  ...
  getCurrentSessionType() {
    ...
    return ctx ? getChatSessionType(ctx.chatSessionResource) : void 0;
  }
  /**
  ...
  computeVisibleOptionGroups() {
    ...
    const delegateSessionType = this.options.sessionTypePickerDelegate?.getActiveSessionProvider?.();
    const effectiveSessionType = delegateSessionType ?? (ctx ? getChatSessionType(ctx.chatSessionResource) : void 0);
      ...
      actionViewItemProvider: /* @__PURE__ */ __name((action, options2) => {
        if (action.id === OpenModelPickerAction.ID && action instanceof MenuItemAction) {
          ...
          const itemDelegate = {
            ...
            showManageModelsAction: /* @__PURE__ */ __name(() => {
              const sessionType = this.getCurrentSessionType();
              return !sessionType || sessionType === localChatSessionType;
            }, "showManageModelsAction"),
            showUnavailableFeatured: /* @__PURE__ */ __name(() => {
              const sessionType = this.getCurrentSessionType();
              return !sessionType || sessionType === localChatSessionType;
      ...
      actionViewItemProvider: /* @__PURE__ */ __name((action, options2) => {
        if ((action.id === OpenSessionTargetPickerAction.ID || action.id === OpenDelegationPickerAction.ID) && action instanceof MenuItemAction) {
          ...
          const delegate2 = {
            ...
            setPermissionLevel: /* @__PURE__ */ __name((level) => {
              this._currentPermissionLevel.set(level, void 0);
              this.permissionLevelKey.set(level);
            }, "setPermissionLevel")
          };
          return this.permissionWidget = this.instantiationService.createInstance(PermissionPickerActionItem, action, delegate2, pickerOptions);
        }
        return void 0;
      }, "actionViewItemProvider")
    }));
...
class HiddenActionViewItem extends BaseActionViewItem {
  static {
    __name(this, "HiddenActionViewItem");
  }
  constructor(action) {
    super(void 0, action);
  }
