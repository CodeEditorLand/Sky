...
class KernelPickerStrategyBase {
  ...
  async _showKernelExtension(extensionWorkbenchService, extensionService, extensionManagementServerService, viewType, extIds, isInsiders) {
    ...
    for (const extId of extIds) {
      ...
      if (extension.enablementState === 10 || extension.enablementState === 11 || extension.enablementState === 2) {
  ...
  async _getKernelRecommendationsQuickPickItems(notebookTextModel, extensionWorkbenchService) {
    ...
    if (suggestedExtension) {
      ...
      const extensions = extensionWorkbenchService.installed.filter((e) => (e.enablementState === 3 || e.enablementState === 12 || e.enablementState === 13) && suggestedExtension.extensionIds.includes(e.identifier.id));
