...
let BranchPicker = class BranchPicker2 extends Disposable {
  ...
  async setRepository(repository) {
    ...
    try {
      ...
      this._preferredBranch = void 0;
      const defaultBranch = (preferred ? this._branches.find((b) => b === preferred) : void 0) ?? this._branches.find((b) => b === repository.state.get().HEAD?.name) ?? this._branches.find((b) => b === "main") ?? this._branches.find((b) => b === "master") ?? this._branches[0];
      if (defaultBranch) {
