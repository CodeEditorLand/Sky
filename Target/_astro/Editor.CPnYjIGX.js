...
import { e as H } from "./editor.main.D0g8BD-5.js";
...
function ve(e, t, n, { on: i, shouldFocus: a = !1 }) {
	y(() => {
		i.includes(
			(
		...
		y(() => {
			i.forEach((N) => {
				...
				(l ? "initialValue" in r : o) &&
					h.initialValue.set(() => (l ? u : T(N, o)));
				const k = v && h.dirty.get();
				!m &&
					!k &&
					(h.startValue.set(h.initialValue.get),
					h.value.set(h.initialValue.get),
					h.elements.get().forEach((B) => {
						B.type === "file" && (B.value = "");
					})),
					W || h.touched.set(!1),
					!q && !m && !k && h.dirty.set(!1),
				...
				a.forEach((N) => {
					...
					!p &&
						!k &&
						(o && h.initialItems.set(T(N, o)?.map(() => I()) || []),
		...
		Promise.all(
			i.map(async (c) => {
				...
				if (!l || y(g.active.get)) {
					...
					const v = m || u[c] || "";
					return v && (o = !1), g.error.set(v), v ? c : null;
				}
			}),
		),
		Promise.all(
...
//# sourceMappingURL=Editor.CPnYjIGX.js.map
