...
let AgentSessionsControl = class AgentSessionsControl2 extends Disposable {
  ...
  constructor(container, options, contextMenuService, contextKeyService, instantiationService, chatSessionsService, commandService, menuService, agentSessionsService, telemetryService, editorService, storageService) {
    super();
  ...
  createList(container) {
    ...
    const sessionRenderer = this._register(this.instantiationService.createInstance(AgentSessionRenderer, {
      ...this.options,
      isGroupedByRepository: /* @__PURE__ */ __name(() => this.options.filter.groupResults?.() === AgentSessionsGrouping.Repository, "isGroupedByRepository")
