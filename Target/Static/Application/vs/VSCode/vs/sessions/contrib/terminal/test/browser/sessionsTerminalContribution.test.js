...
function makeTerminalInstance(id, cwd) {
  ...
  const capabilities = {
    get(cap) {
      if (cap === 2 && commandHistory.length > 0) {
  ...
  return {
    instanceId: id,
    isDisposed: false,
    getInitialCwd: /* @__PURE__ */ __name(() => Promise.resolve(cwd), "getInitialCwd"),
    capabilities,
    _testCommandHistory: commandHistory
  };
}
__name(makeTerminalInstance, "makeTerminalInstance");
function addCommandToInstance(instance, timestamp) {
  instance._testCommandHistory.push({ timestamp });
}
__name(addCommandToInstance, "addCommandToInstance");
suite("SessionsTerminalContribution", () => {
  ...
  test("creates new terminal when switching to a different background path", async () => {
    ...
    activeSessionObs.set(makeAgentSession({ worktree: worktree1, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
    activeSessionObs.set(makeAgentSession({ worktree: worktree2, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
  ...
  test("switching back to a previously used background path reuses the existing terminal", async () => {
    ...
    activeSessionObs.set(makeAgentSession({ worktree: cwd1, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
    assert.strictEqual(createdTerminals.length, 1);
    activeSessionObs.set(makeAgentSession({ worktree: cwd2, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
    assert.strictEqual(createdTerminals.length, 2);
    activeSessionObs.set(makeAgentSession({ worktree: cwd1, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
  ...
  test("only terminals of the active session are visible after multiple switches", async () => {
    ...
    activeSessionObs.set(makeAgentSession({ worktree: cwd2, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
    activeSessionObs.set(makeAgentSession({ worktree: cwd3, providerType: AgentSessionProviders.Background }), void 0);
    await tick();
