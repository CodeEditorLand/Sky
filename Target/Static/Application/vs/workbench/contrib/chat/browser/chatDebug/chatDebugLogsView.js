...
let ChatDebugLogsView = class ChatDebugLogsView2 extends Disposable {
  ...
  constructor(parent, filterState, chatService, chatDebugService, instantiationService, contextKeyService) {
    super();
  ...
  refreshList() {
    ...
    if (filterText) {
      ...
      filtered = filtered.filter((e) => {
        const matchesText = /* @__PURE__ */ __name((term) => {
          ...
          switch (e.kind) {
            ...
            case "userMessage":
              return e.message.toLowerCase().includes(term) || e.sections.some((s) => s.name.toLowerCase().includes(term) || s.content.toLowerCase().includes(term));
            case "agentResponse":
              return e.message.toLowerCase().includes(term) || e.sections.some((s) => s.name.toLowerCase().includes(term) || s.content.toLowerCase().includes(term));
          }
        }, "matchesText");
