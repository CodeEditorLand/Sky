...
class ChatResponseAccessibleProvider extends Disposable {
  ...
  _getContent(item) {
    ...
    if (isResponseVM(item)) {
      const toolInvocation = item.response.value.find((item2) => item2.kind === "toolInvocation");
      if (toolInvocation?.confirmationMessages) {
        ...
        const terminalCommand = toolInvocation.toolSpecificData && "command" in toolInvocation.toolSpecificData ? toolInvocation.toolSpecificData.command : void 0;
