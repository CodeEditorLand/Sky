    ...
    this._register(this.languageFeaturesService.completionProvider.register({ scheme: Schemas.vscodeChatInput, hasAccessToAllModels: true }, {
      ...
      provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, _token) => {
        ...
        if (widget.lockedAgentId) {
          ...
          customAgentTarget = (ctx ? chatSessionsService.getCustomAgentTargetForSessionType(getChatSessionType(sessionResource)) : void 0) ?? Target.Undefined;
        }
    ...
    this._register(this.languageFeaturesService.completionProvider.register({ scheme: Schemas.vscodeChatInput, hasAccessToAllModels: true }, {
      ...
      provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, token) => {
        ...
        const userInvocableCommands = promptCommands.filter((c) => {
          ...
          return true;
        }).filter((c) => c.parsedPromptFile?.header?.userInvocable !== false).filter((c) => !c.when || widget.scopedContextKeyService.contextMatchesRules(c.when));
