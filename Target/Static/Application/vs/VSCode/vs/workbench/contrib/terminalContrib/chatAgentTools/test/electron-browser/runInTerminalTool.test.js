...
class TestRunInTerminalTool extends RunInTerminalTool {
  static {
    __name(this, "TestRunInTerminalTool");
  }
  constructor() {
    super(...arguments);
    this._osBackend = Promise.resolve(
      1
...
suite("RunInTerminalTool", () => {
  ...
  setup(() => {
    ...
    setConfig("chat.tools.terminal.enableAutoApprove", true);
    setConfig("chat.tools.terminal.blockDetectedFileWrites", "outsideWorkspace");
    ...
    instantiationService.stub(IChatService, {
      ...
      getSession: /* @__PURE__ */ __name(() => void 0, "getSession")
    });
    ...
    instantiationService.stub(IHistoryService, {
      getLastActiveWorkspaceRoot: /* @__PURE__ */ __name(() => void 0, "getLastActiveWorkspaceRoot")
    });
    ...
    instantiationService.stub(ITerminalService, {
      ...
      setNextCommandId: /* @__PURE__ */ __name(async () => {
      }, "setNextCommandId")
    });
    instantiationService.stub(ITerminalProfileResolverService, {
      getDefaultProfile: /* @__PURE__ */ __name(async () => ({ path: "bash" }), "getDefaultProfile")
    });
  ...
  function setAutoApprove(value) {
    setConfig("chat.tools.terminal.autoApprove", value);
  }
  __name(setAutoApprove, "setAutoApprove");
  ...
  __name(clearAutoApproveWarningAcceptedState, "clearAutoApproveWarningAcceptedState");
  ...
  __name(executeToolTest, "executeToolTest");
  ...
  __name(assertAutoApproved, "assertAutoApproved");
  ...
  __name(assertConfirmationRequired, "assertConfirmationRequired");
  suite("default auto-approve rules", () => {
    const defaults = terminalChatAgentToolsConfiguration[
  ...
  suite("prepareToolInvocation - custom actions for dropdown", () => {
    ...
    __name(assertDropdownActions, "assertDropdownActions");
    ...
    test("should prevent auto approval when writing to a file outside the workspace", async () => {
      ...
      instantiationService.stub(IHistoryService, {
        getLastActiveWorkspaceRoot: /* @__PURE__ */ __name(() => workspaceFolder, "getLastActiveWorkspaceRoot")
      });
      const result = await executeToolTest({
  ...
  suite("chat session disposal cleanup", () => {
    const createMockTerminal = /* @__PURE__ */ __name((processId) => ({
    ...
    test("should restore all terminals into the session terminal map and dispose them when archived", () => {
      ...
      chatSessionArchivedEmitter.fire({
        ...
        isArchived: /* @__PURE__ */ __name(() => true, "isArchived")
      });
    ...
    test("should dispose all terminals associated with a single chat session when archived", () => {
      ...
      const sessionResource = LocalChatSessionUri.forSession(sessionId);
      const mockTerminal1 = { dispose: /* @__PURE__ */ __name(() => {
      }, "dispose"), processId: 33333 };
      const mockTerminal2 = { dispose: /* @__PURE__ */ __name(() => {
      }, "dispose"), processId: 44444 };
      let terminal1Disposed = false;
      let terminal2Disposed = false;
  ...
  suite("auto approve warning acceptance mechanism", () => {
    ...
    test("should require confirmation when auto-approve disabled regardless of warning acceptance", async () => {
      setConfig("chat.tools.terminal.enableAutoApprove", false);
      setAutoApprove({
        echo: true
      });
      const result = await executeToolTest({ command: "echo hello world" });
  ...
  suite("TerminalProfileFetcher", () => {
    suite("getCopilotProfile", () => {
      (isWindows ? test : test.skip)("should return custom profile when configured", async () => {
        runInTerminalTool.setBackendOs(
          1
          /* OperatingSystem.Windows */
        );
        const customProfile = Object.freeze({ path: "C:\\Windows\\System32\\powershell.exe", args: ["-NoProfile"] });
        setConfig("chat.tools.terminal.terminalProfile.windows", customProfile);
      ...
      (isLinux ? test : test.skip)("should fall back to default shell when no custom profile is configured", async () => {
        ...
        setConfig("chat.tools.terminal.terminalProfile.linux", null);
        const result = await runInTerminalTool.profileFetcher.getCopilotProfile();
  ...
  suite("denial info in disclaimers", () => {
    ...
    __name(getDisclaimerValue, "getDisclaimerValue");
