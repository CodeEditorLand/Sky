...
var require_common = __commonJS({
  "../../Dependency/Microsoft/Dependency/Editor/out/vs/sessions/test/e2e/common.cjs"(exports, module) {
    const fs = require("fs");
    ...
    __name(parseScenario, "parseScenario");
    function discoverScenarios() {
      return fs.readdirSync(SCENARIOS_DIR).filter((f) => f.endsWith(".scenario.md")).sort().map((f) => parseScenario(path.join(SCENARIOS_DIR, f)));
    }
    __name(discoverScenarios, "discoverScenarios");
    ...
    __name(runPlaywrightCli, "runPlaywrightCli");
    function getSnapshot() {
      ...
      if (fileMatch) {
        ...
        try {
          ...
          if (!content.trim()) {
            console.error(`  [snapshot] file is empty`);
          }
    ...
    __name(waitForServer, "waitForServer");
    ...
    __name(commandsPathForScenario, "commandsPathForScenario");
    module.exports = {
      APP_ROOT,
      SCENARIOS_DIR,
      parseScenario,
