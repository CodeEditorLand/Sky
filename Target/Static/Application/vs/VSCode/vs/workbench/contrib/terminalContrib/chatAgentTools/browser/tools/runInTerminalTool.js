...
let RunInTerminalTool = class RunInTerminalTool2 extends Disposable {
  ...
  constructor(_chatService, _configurationService, _historyService, _instantiationService, _labelService, _languageModelToolsService, _remoteAgentService, _storageService, _terminalChatService, _logService, _terminalService, _workspaceContextService, _chatWidgetService, _agentSessionsService) {
    super();
  ...
  async handleToolStream(context, _token) {
    ...
    if (partialInput && typeof partialInput === "object" && partialInput.command) {
      ...
      const invocationMessage = partialInput.isBackground ? new MarkdownString(localize("runInTerminal.streaming.background", "Running `{0}` in background", escapedCommand)) : new MarkdownString(localize("runInTerminal.streaming", "Running `{0}`", escapedCommand));
  ...
  async prepareToolInvocation(context, token) {
    ...
    if (isSessionAutoApproved && deniedAnalyzerResult?.denialDetails && context.forceConfirmationReason === void 0) {
      const denial = deniedAnalyzerResult.denialDetails;
      const deniedRule = denial.ruleSourceText ? ` Rule: \`${escapeMarkdownSyntaxTokens(denial.ruleSourceText)}\`.` : "";
      ...
      toolSpecificData.alternativeRecommendation = shouldCircuitBreak ? `POLICY_DENIED_CIRCUIT_BREAKER: Command was blocked ${deniedAttempts} times in this session and will not be retried. Scope: ${denial.scope}. Command: \`${escapeMarkdownSyntaxTokens(denial.deniedCommand)}\`. Reason: ${denial.reason}.${deniedRule}` : `POLICY_DENIED: Command was not executed in auto-approval session mode. Scope: ${denial.scope}. Command: \`${escapeMarkdownSyntaxTokens(denial.deniedCommand)}\`. Reason: ${denial.reason}.${deniedRule}`;
      return {
        confirmationMessages: void 0,
        toolSpecificData
      };
    ...
    const shouldShowConfirmation = !isFinalAutoApproved && !isSessionAutoApproved || context.forceConfirmationReason !== void 0;
    ...
    const invocationMessage = args.isBackground ? new MarkdownString(localize("runInTerminal.invocation.background", "Running `{0}` in background", escapedDisplayCommand)) : new MarkdownString(localize("runInTerminal.invocation", "Running `{0}`", escapedDisplayCommand));
    return {
      invocationMessage,
