...
let ExtensionManagementService = class ExtensionManagementService2 extends AbstractExtensionManagementService {
  static {
    ...
    try {
      ...
      if (!new ExtensionKey(gallery.identifier, gallery.version).equals(new ExtensionKey({ id: getGalleryExtensionId(manifest.publisher, manifest.name) }, manifest.version))) {
        throw new ExtensionManagementError(
          nls.localize("invalidManifest", "Cannot install '{0}' extension because of manifest mismatch with Marketplace", gallery.identifier.id),
          "Invalid"
  ...
  async downloadExtension(extension, operation, verifySignature, clientTargetPlatform) {
    ...
    if (verificationStatus !== ExtensionSignatureVerificationCode.Success && !(verificationStatus === ExtensionSignatureVerificationCode.NotSigned && !shouldRequireSignature) && verifySignature && this.environmentService.isBuilt) {
      try {
        await this.extensionsDownloader.delete(location);
      } catch (e) {
        this.logService.warn(`Error while deleting the downloaded file`, location.toString(), getErrorMessage(e));
      ...
      if (!verificationStatus) {
        throw new ExtensionManagementError(
          nls.localize("signature verification not executed", "Signature verification was not executed."),
      ...
      switch (verificationStatus) {
        ...
        case ExtensionSignatureVerificationCode.NotSigned:
          throw new ExtensionManagementError(
            nls.localize("signature verification failed", "Signature verification failed with '{0}' error.", verificationStatus),
...
let ExtensionsScanner = class ExtensionsScanner2 extends Disposable {
  ...
  async extractUserExtension(extensionKey, zipPath, removeIfExists, token) {
    ...
    if (await this.fileService.exists(extensionLocation)) {
      ...
      try {
        ...
        throw new ExtensionManagementError(
          nls.localize("errorDeleting", "Unable to delete the existing folder '{0}' while installing the extension '{1}'. Please delete the folder manually and try again", extensionLocation.fsPath, extensionKey.id),
          "Delete"
  ...
  async scanLocalExtension(location, type, profileLocation) {
    try {
      ...
      throw new ExtensionManagementError(
        nls.localize("cannot read", "Cannot read the extension from {0}", location.path),
        "ScanningExtension"
...
let InstallExtensionInProfileTask = class InstallExtensionInProfileTask2 extends AbstractExtensionTask {
  static {
