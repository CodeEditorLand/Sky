...
let PlaywrightService = class PlaywrightService2 extends Disposable {
  ...
  async captureScreenshot(pageId, selector, fullPage) {
    await this.initialize();
    return this._pages.runAgainstPage(pageId, async (page) => {
      const screenshotBuffer = selector ? await page.locator(selector).screenshot({ type: "jpeg", quality: 80 }) : await page.screenshot({ type: "jpeg", quality: 80, fullPage: fullPage ?? false });
