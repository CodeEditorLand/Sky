...
let TerminalTabList = class TerminalTabList2 extends WorkbenchList {
  ...
  layout(height, width) {
    ...
    const newHasText = actualWidth >= 63;
    const newHasActionBar = actualWidth > 105;
...
let TerminalTabsRenderer = class TerminalTabsRenderer2 {
  ...
  renderTemplate(container) {
    ...
    const actionBar = templateDisposables.add(new ActionBar(actionsContainer, {
      ...
      actionViewItemProvider: /* @__PURE__ */ __name((action, options) => action instanceof MenuItemAction ? templateDisposables.add(this._instantiationService.createInstance(MenuEntryActionViewItem, action, { hoverDelegate: options.hoverDelegate })) : void 0, "actionViewItemProvider")
    }));
    return {
      element,
      label,
  ...
  fillActionBar(instance, template) {
    const actions = [
      template.elementDisposables.add(new Action("workbench.action.terminal.splitActiveTab", terminalStrings.split.short, ThemeIcon.asClassName(Codicon.splitHorizontal), true, async () => {
    ...
    actions.push(template.elementDisposables.add(new Action("workbench.action.terminal.killActiveTab", terminalStrings.kill.short, ThemeIcon.asClassName(Codicon.trashcan), true, async () => {
...
__name(isTerminalInstance, "isTerminalInstance");
export {
  TerminalTabList,
