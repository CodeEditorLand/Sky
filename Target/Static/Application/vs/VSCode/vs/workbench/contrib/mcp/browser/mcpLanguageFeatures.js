...
let McpLanguageFeatures = class McpLanguageFeatures2 extends Disposable {
  ...
  async _provideCodeLenses(model, onDidChangeCodeLens) {
    ...
    const lenses = [];
    const lensList = { lenses, dispose: /* @__PURE__ */ __name(() => store.dispose(), "dispose") };
  ...
  async _provideInlayHints(model, range) {
    ...
    function pushAnnotation(savedId, offset, saved) {
      const tooltip = new MarkdownString([
        createMarkdownCommandLink({ id: "workbench.mcp.editStoredInput", text: localize("edit", "Edit"), arguments: [savedId, model.uri, mcpConfigurationSection, inConfig.target], tooltip: localize("edit.savedValue.tooltip", "Edit saved value") }),
        createMarkdownCommandLink({ id: "workbench.mcp.removeStoredInput", text: localize("clear", "Clear"), arguments: [inConfig.scope, savedId], tooltip: localize("clear.savedValue.tooltip", "Clear saved value") }),
        createMarkdownCommandLink({ id: "workbench.mcp.removeStoredInput", text: localize("clearAll", "Clear All"), arguments: [inConfig.scope], tooltip: localize("clearAll.savedValues.tooltip", "Clear all saved values") })
      ].join(" | "), { isTrusted: true });
      const hint = {
