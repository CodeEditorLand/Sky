...
let CancellableRequest = class CancellableRequest2 {
  ...
  constructor(cancellationTokenSource, requestId, responseCompletePromise, toolsService) {
...
let ChatService = class ChatService2 extends Disposable {
  ...
  async loadRemoteSession(sessionResource, location, token) {
    ...
    if (providedSession.progressObs && lastRequest && providedSession.interruptActiveResponseCallback) {
      const initialCancellationRequest = this.instantiationService.createInstance(CancellableRequest, new CancellationTokenSource(), void 0, void 0);
        ...
        return token2.onCancellationRequested(() => {
          providedSession.interruptActiveResponseCallback?.().then((userConfirmedInterruption) => {
            if (!userConfirmedInterruption) {
              const newCancellationRequest = this.instantiationService.createInstance(CancellableRequest, new CancellationTokenSource(), void 0, void 0);
  ...
  _sendRequestAsync(model, sessionResource, parsedRequest, attempt, enableCommandDetection, defaultAgent, location, options) {
    ...
    const cancellableRequest = this.instantiationService.createInstance(CancellableRequest, source, void 0, rawResponsePromise);
